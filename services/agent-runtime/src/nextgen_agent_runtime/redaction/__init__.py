"""Sanitisation applied before anything crosses the service boundary.

Secrets, hidden model reasoning and unnecessary personal data must not enter
prompts, persisted traces, logs or model-visible tool results. Redaction runs
on the runtime side before transport and again in Frappe before persistence:
...
# Field names whose value is never useful for audit and always dangerous to keep.
SECRET_KEY_PARTS = (
    "api_key",
    "apikey",
    "access_token",
...
# Model-internal reasoning is never persisted, only bounded metadata.
...
def redact_text(value: str) -> str:
    """Mask credential-shaped and directly identifying substrings in free text."""
...
def redact_error(error: BaseException | str, *, limit: int = 500) -> str:
    ...
    Tracebacks stay in the runtime's own logs; the persisted ERP audit record
    only needs the classification and a short bounded message.
    """
