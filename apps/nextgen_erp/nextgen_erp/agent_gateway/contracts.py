"""Versioned service contracts shared with the Agent Runtime microservice.

This is the Frappe-side copy of the boundary. The runtime keeps an identical
copy in ``nextgen_agent_runtime.models.contracts`` so the two services stay
independently deployable and neither imports the other.
``contract_fingerprint`` lets the contract tests on both sides prove the copies
still describe the same payloads.
...
Payload shapes are closed (``additionalProperties: false``). An unknown key is
contract drift, and drift fails at the boundary instead of being forwarded into
ERP.
"""
